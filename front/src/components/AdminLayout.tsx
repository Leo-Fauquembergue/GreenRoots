...
import { Home, ShoppingCart, Users, TreePine, LayoutDashboard } from 'lucide-react';

// Classe de base pour les liens de la sidebar
const sidebarLinkClasses = "flex items-center gap-3 p-3 rounded-md transition-colors text-left";
  ...
  return (
    <div className="flex min-h-screen font-sans bg-gray-100">
      ...
      <aside className="w-64 bg-[#152416] text-white p-6 flex flex-col flex-shrink-0">
        <h2 className="text-2xl font-bold mb-8 text-center greenroots">GreenRoots</h2>
        <nav className="flex flex-col gap-2">
          <NavLink
            to="/admin"
            end // 'end' est important pour que le lien ne reste pas actif sur les sous-pages
            className={({ isActive }) => `${sidebarLinkClasses} ${isActive ? activeLinkClasses : inactiveLinkClasses}`}
          >
        ...
        <Link to="/" className={`${sidebarLinkClasses} ${inactiveLinkClasses} mt-auto`}>
      ...
      <div className="flex-1 flex flex-col">
        <header className="bg-white p-4 shadow-md flex justify-between items-center">
          ...
          {user && <p className="text-sm text-gray-600">Connecté en tant que <strong>{user.name}</strong></p>}
        </header>
