...
export default function AdminTreeTracking() {
	...
	const fetchTreeAndTrackings = useCallback(async () => {
		...
		try {
			...
			setPlantingDate(
				fetchedTree.plantingDate ? fetchedTree.plantingDate.split("T")[0] : "",
			);
	...
	const handleSavePlantingInfo = async (e: React.FormEvent) => {
		...
		try {
			...
			toastRef.current?.showToast(
				"Informations de plantation mises à jour !",
				"success",
			);
			await fetchTreeAndTrackings();
		} catch (error: any) {
	...
	return (
		<div className="bg-white p-6 rounded-lg shadow-md space-y-8">
			<div>
				<Link
					to="/admin/orders"
					className="inline-flex items-center gap-2 text-emerald-600 hover:underline mb-4 font-semibold"
				>
				...
				<p className="text-gray-600">
					Pour :{" "}
					<strong>{tree.personalName || tree.catalogTree.commonName}</strong>{" "}
					(ID: {tree.plantedTreeId})
				</p>
			</div>

			{/* Formulaire des Informations de Plantation */}
			<form
				onSubmit={handleSavePlantingInfo}
				className="p-4 border rounded-lg bg-gray-50 space-y-4"
			>
				<h3 className="font-semibold text-xl text-gray-800 flex items-center gap-3">
					<MapPin size={20} /> Informations de Plantation
				</h3>
				<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
					...
					<div>
						...
						<input
							id="plantingPlace"
							value={plantingPlace}
							onChange={(e) => setPlantingPlace(e.target.value)}
			...
			>
				<h3 className="font-semibold text-xl text-gray-800 flex items-center gap-3">
					<CalendarDays size={20} /> Suivi de l'Arbre
				</h3>
				<h4 className="text-md font-medium text-gray-600 -mt-2">
					{isEditing
						? `Modification du suivi #${isEditing.trackingId}`
			...
			<div>
				<h3 className="font-semibold text-lg mb-4 text-gray-800">
					Historique des suivis ({tree.trackings?.length || 0})
				</h3>
						...
						tree.trackings.map((tracking) => (
							...
							>
								<div>
									<p>
										<strong>Date :</strong>{" "}
										{new Date(tracking.statementDate).toLocaleDateString(
											"fr-FR",
										)}
									</p>
									<p>
										<strong>Condition :</strong>{" "}
										{tracking.condition || "Non renseignée"}
									</p>
									<p>
										<strong>Hauteur :</strong>{" "}
										{tracking.currentHeight
											? `${tracking.currentHeight} cm`
											: "Non renseignée"}
									</p>
								</div>
								<button
