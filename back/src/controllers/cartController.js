import { Order, PlantedTree, CatalogTree, User } from "./../models/associations.js";
...
export async function getCart(req, res) {
  if (!req.session.user) {
    throw new HttpError(401, "Vous devez être connecté pour voir votre panier.");
  }
...
export async function addToCart(req, res) {
  if (!req.session.user) {
    throw new HttpError(401, "Vous devez être connecté pour ajouter un article.");
  }
  const userId = req.session.user.id;
...
export async function deleteFromCart(req, res) {
  if (!req.session.user) {
    throw new HttpError(401, "Vous devez être connecté pour supprimer un article.");
  }
  const userId = req.session.user.id;
  ...
  if (!itemToDelete) {
    throw new HttpError(404, "Cet article n'a pas été trouvé dans votre panier.");
  }
