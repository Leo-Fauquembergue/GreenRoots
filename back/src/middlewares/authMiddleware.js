...
export function isAuthenticated(req, res, next) {
  if (req.session.user) {
    ...
    throw new HttpError(401, "Accès non autorisé. Vous devez être connecté pour accéder à cette ressource.");
  }
}
...
export function isAdmin(req, res, next) {
  if (req.session.user.role === 'admin') {
    ...
    throw new HttpError(403, "Accès refusé. Cette action requiert des droits d'administrateur.");
  }
}